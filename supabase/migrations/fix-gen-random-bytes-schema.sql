-- Bug: pgcrypto (gen_random_bytes) instala no schema `extensions` no
-- Supabase, não em `public`. Funções com SET search_path = public
-- restringem a resolução a esse schema só, então `gen_random_bytes(...)`
-- não resolvia — nunca foi pego porque toda criação de competição até
-- agora passou por SQL direto (migrations/seeds), nunca pela RPC real.
-- Qualifica a chamada explicitamente em vez de abrir o search_path
-- (mais seguro contra search_path hijacking em SECURITY DEFINER).
