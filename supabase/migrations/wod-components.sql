-- Add components array to competition_wods for MAX LOAD multi-component WODs
ALTER TABLE competition_wods
  ADD COLUMN IF NOT EXISTS components TEXT[] DEFAULT NULL;

-- Replace create_competition_wod to accept optional components
...
BEGIN
  ...
  INSERT INTO competition_wods
    (competition_id, name, description, score_type, score_order, cap, order_index, status, components)
  VALUES
    (p_competition_id, trim(p_name), NULLIF(trim(p_description), ''),
     p_score_type, p_score_order, NULLIF(trim(p_cap), ''), v_order, 'draft',
     CASE WHEN array_length(p_components, 1) > 0 THEN p_components ELSE NULL END)
...
GRANT EXECUTE ON FUNCTION create_competition_wod(UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT[]) TO authenticated;
