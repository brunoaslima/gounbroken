-- Recria create_competition gerando public_slug server-side (6 chars [a-z0-9])
-- com retry em caso de colisão (unique constraint).
...
-- Backfill existing competitions that have no slug
DO $$
DECLARE
  r RECORD;
  v_slug TEXT;
  v_tries INT;
BEGIN
  FOR r IN SELECT id FROM competitions WHERE public_slug IS NULL LOOP
    v_tries := 0;
    LOOP
      v_tries := v_tries + 1;
      IF v_tries > 10 THEN
        RAISE EXCEPTION 'slug backfill failed for competition %', r.id;
      END IF;
      v_slug := substring(lower(encode(gen_random_bytes(4), 'hex')) FROM 1 FOR 6);
      EXIT WHEN NOT EXISTS (SELECT 1 FROM competitions WHERE public_slug = v_slug);
    END LOOP;
    UPDATE competitions SET public_slug = v_slug WHERE id = r.id;
  END LOOP;
END;
$$;
