-- Per-division scoring: N = total approved teams in the division, points = N - rank + 1.
-- Drops both existing overloads (1-param and 2-param) before recreating as a single
-- canonical version with optional p_division_id filter.
...
BEGIN
  ...
  WITH
  division_sizes AS (
    -- N per division = all approved teams, including those without results yet
  ...
  wod_rankings AS (
    -- Rank within (wod, division) so divisions compete only against each other
    SELECT
      r.team_id,
      r.wod_id,
      r.raw_result,
